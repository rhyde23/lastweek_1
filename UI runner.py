...
def get_best_players_in_given_position(team_viewing, position) :
    ...
    return sorted(team_players_dict, key=lambda x: team_players_dict[x]['Positions'][position], reverse=True)
...
"""
print(myfont3.render('Rating:75', True, cyan).get_width())
quit()
"""
...
def display_player_names() :
    positions_in_order = get_positions_from_formation(current_data['CurrentFormation'])
...
def get_hover_square_coords(x, y) :
    for pos_key in jersey_coords :
        ...
        if x >= pos_coordinates_x and x <= pos_coordinates_x+72 and y >= pos_coordinates_y and y <= pos_coordinates_y+72 :
            #print(True, (pos_coordinates_x, pos_coordinates_y), pos_key)
...
#print(get_best_players_in_given_position('Manchester City', 'CM'))
#quit()
